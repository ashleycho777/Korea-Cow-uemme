...
Layout:
  +----------------------------------------------------------+
  | [Sort: Block ID v]  [Group by Bay ]  [Show due dates ]   |
  +--------+-------------------------------------------------+
  | Block  |  time axis  ->                                   |
  |  0     |  ----------------   due^ T=3                     |
  |  1     |       ----------                                |
...
class GanttCanvas(QWidget):
    ...
    def _draw_row(self, p: QPainter, ri: int, row: dict):
        ...
        # -- Release marker (right-pointing triangle on bar left edge) --------
...
class GanttTab(QWidget):
    ...
    def _update_legend(self):
        ...
        for color, label in [
            ...
            ("#16a34a", "v Due (on-time)"),
            ("#dc2626", "v Due (tardy) + hatch"),
        ]:
            swatch = QLabel()
            swatch.setFixedSize(14, 14)
    ...
    def _update_info(self):
        ...
        self._lbl_info.setText(
            f"{n} blocks  |  {n_tardy} tardy  |  SumT={total_t:.1f}  "
